...
(() => {
  ...
  // node_modules/.pnpm/@codemirror+view@6.10.0/node_modules/@codemirror/view/dist/index.js
  ...
  function scanCompositionTree(pos, side, view2, text, dLen, enterView, fromText) {
  ...
  function posFromDOMInCompositionTree(node, offset, view2, text, dLen) {
    if (view2 instanceof MarkView) {
      let pos = 0;
      for (let child = view2.dom.firstChild; child; child = child.nextSibling) {
  ...
  function posAtCoords(view2, coords, precise, bias = -1) {
    var _a, _b;
    let content = view2.contentDOM.getBoundingClientRect(), docTop = content.top + view2.viewState.paddingTop;
    ...
    if (lineStart < view2.viewport.from)
      return view2.viewport.from == 0 ? 0 : precise ? null : posAtCoordsImprecise(view2, content, block, x, y);
    if (lineStart > view2.viewport.to)
      return view2.viewport.to == view2.state.doc.length ? view2.state.doc.length : precise ? null : posAtCoordsImprecise(view2, content, block, x, y);
    ...
    let node, offset = -1;
    if (element && ((_a = view2.docView.nearest(element)) === null || _a === void 0 ? void 0 : _a.isEditable) != false) {
  ...
  function posAtCoordsImprecise(view2, contentRect, block, x, y) {
    ...
    let content = view2.state.sliceDoc(block.from, block.to);
    return block.from + findColumn(content, into, view2.state.tabSize);
  }
  ...
  var InputState = class {
      ...
      view2.scrollDOM.addEventListener("mousedown", (event) => {
        if (event.target == view2.scrollDOM && event.clientY > view2.contentDOM.getBoundingClientRect().bottom) {
          ...
          if (!event.defaultPrevented && event.button == 2) {
            let start = view2.contentDOM.style.minHeight;
            view2.contentDOM.style.minHeight = "100%";
            setTimeout(() => view2.contentDOM.style.minHeight = start, 200);
          }
        }
      });
      view2.scrollDOM.addEventListener("drop", (event) => {
        if (event.target == view2.scrollDOM && event.clientY > view2.contentDOM.getBoundingClientRect().bottom)
    ...
    ensureHandlers(view2, plugins) {
      ...
      for (let plugin of plugins)
        if (handlers2 = (_a = plugin.update(view2).spec) === null || _a === void 0 ? void 0 : _a.domEventHandlers) {
  ...
  handlers.beforeinput = (view2, event) => {
    ...
    if (browser.chrome && browser.android && (pending = PendingKeys.find((key) => key.inputType == event.inputType))) {
      ...
      if (pending.key == "Backspace" || pending.key == "Delete") {
        ...
        setTimeout(() => {
          var _a2;
          if ((((_a2 = window.visualViewport) === null || _a2 === void 0 ? void 0 : _a2.height) || 0) > startViewHeight + 10 && view2.hasFocus) {
